...
static void isInitSource(j_decompress_ptr dinfo) {
  ...
  src->buffer = (JOCTET*)(*dinfo->mem->alloc_small)(
      (j_common_ptr)dinfo, JPOOL_PERMANENT, kStreamBufferSize * sizeof(JOCTET));
  if (src->buffer == nullptr) {
    jpegSafeThrow(
        (j_common_ptr)dinfo, "Failed to allocate memory for read buffer");
  }
}
...
static boolean isFillInputBuffer(j_decompress_ptr dinfo) {
  ...
  JNIEnv* env = src->env;
  jint nbytes =
      env->CallIntMethod(src->inputStream, midInputStreamRead, src->javaBuffer);
  jpegJumpOnException((j_common_ptr)dinfo);
  ...
  if (nbytes <= 0) {
    ...
    env->GetByteArrayRegion(
        src->javaBuffer, 0, kStreamBufferSize, (jbyte*)src->buffer);
    jpegJumpOnException((j_common_ptr)dinfo);
  }
...
static void isSkipInputData(j_decompress_ptr dinfo, long num_bytes) {
  ...
  if (num_bytes > 0) {
    if (src->public_fields.bytes_in_buffer > (unsigned long)num_bytes) {
      ...
      // TODO: 3752653
      env->CallLongMethod(src->inputStream, midInputStreamSkip, (jlong)to_skip);
      jpegJumpOnException((j_common_ptr)dinfo);
...
JpegInputStreamWrapper::JpegInputStreamWrapper(JNIEnv* env, jobject inputStream)
    : inputStream(inputStream), env(env) {
  ...
  public_fields.resync_to_restart =
...
static void osInitDestination(j_compress_ptr cinfo) {
  ...
  if (dest->javaBuffer == NULL) {
    jpegSafeThrow(
        (j_common_ptr)cinfo, "Failed to allocate memory for java byte buffer.");
  }
  jpegJumpOnException((j_common_ptr)cinfo);

  // allocate the output buffer --- it will be released when done with image
  dest->buffer = (JOCTET*)(*cinfo->mem->alloc_small)(
      (j_common_ptr)cinfo, JPOOL_IMAGE, kStreamBufferSize * sizeof(JOCTET));
  if (dest->buffer == NULL) {
    jpegSafeThrow(
        (j_common_ptr)cinfo, "Failed to allocate memory for byte buffer.");
  }
...
static boolean osEmptyOutputBuffer(j_compress_ptr cinfo) {
  ...
  env->SetByteArrayRegion(
      dest->javaBuffer, 0, kStreamBufferSize, (jbyte*)dest->buffer);
  jpegJumpOnException((j_common_ptr)cinfo);
  env->CallVoidMethod(
      dest->outputStream, midOutputStreamWrite, dest->javaBuffer);
  jpegJumpOnException((j_common_ptr)cinfo);
  dest->public_fields.next_output_byte = dest->buffer;
  dest->public_fields.free_in_buffer = kStreamBufferSize;
  return true;
...
static void osTermDestination(j_compress_ptr cinfo) {
  ...
  if (datacount > 0) {
    env->SetByteArrayRegion(
        dest->javaBuffer, 0, datacount, (jbyte*)dest->buffer);
    jpegJumpOnException((j_common_ptr)cinfo);
    env->CallVoidMethod(
        dest->outputStream,
...
JpegOutputStreamWrapper::JpegOutputStreamWrapper(
    ...
    : outputStream(output_stream), env(env) {
