...
class ProducerToDataSourceAdapterTest {
  private val requestListener: RequestListener2 = mock()
  private lateinit var result1: Any
  private lateinit var result2: Any
  private lateinit var result3: Any
  private lateinit var exception: Exception
  private val dataSubscriber1: DataSubscriber<Any> = mock()
  private val dataSubscriber2: DataSubscriber<Any> = mock()
  private lateinit var settableProducerContext: SettableProducerContext
  private val producer: Producer<Any> = mock()
  private lateinit var internalConsumer: Consumer<Any>
  ...
  fun setUp() {
    ...
    whenever(settableProducerContext.id).thenReturn(REQUEST_ID)
    whenever(settableProducerContext.isPrefetch).thenReturn(true)

    dataSource =
        ProducerToDataSourceAdapter.create(producer, settableProducerContext, requestListener)

    val captor = argumentCaptor<Consumer<Any>>()
  ...
  private fun verifyState(
    ...
    assertThat(dataSource.result).isSameAs(result)
  ...
  private fun verifyWithResult(result: Any?, isLast: Boolean) {
    verifyState(isLast, result != null, result, NOT_FAILED, null)
    verifyNoMoreInteractionsAndReset()
  }

  private fun verifyFailed(result: Any?, throwable: Throwable?) {
  ...
  private fun testNewResult(result: Any?, isLast: Boolean, numSubscribers: Int) {
    internalConsumer.onNewResult(result, BaseConsumer.simpleStatusForIsLast(isLast))
    if (isLast) {
  ...
  private fun testFailure(result: Any?, numSubscribers: Int) {
  ...
  fun test_C_I_a() {
    testClose(NOT_FINISHED, 1)
    internalConsumer.onNewResult(result2, Consumer.NO_FLAGS)
  ...
  fun test_C_L_a() {
    testClose(NOT_FINISHED, 1)
    internalConsumer.onNewResult(result2, Consumer.IS_LAST)
    verifyClosed(NOT_FINISHED, null)
    testSubscribe(NO_INTERACTIONS)
  }
  ...
  @Test
  fun test_I_a_C() {
    testNewResult(result1, INTERMEDIATE, 1)
  ...
  fun test_I_I_a_C() {
    testNewResult(result1, INTERMEDIATE, 1)
    testNewResult(result2, INTERMEDIATE, 1)
    testSubscribe(ON_NEW_RESULT)
    testClose(NOT_FINISHED, 2)
  }
  ...
  fun test_F_I_a_C() {
    testFailure(null, 1)
    internalConsumer.onNewResult(result1, Consumer.NO_FLAGS)
  ...
  fun test_F_L_a_C() {
    testFailure(null, 1)
    internalConsumer.onNewResult(result1, Consumer.IS_LAST)
    verifyFailed(null, exception)
    testSubscribe(ON_FAILURE)
    testClose(exception)
  }
