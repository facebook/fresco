...
public class MultiplexProducerTest {
  ...
  public void testSingleRequest() {
    ...
    verify(mConsumer1).onNewResult(
        argThat(new CloseableReferenceMatcher(mIntermediateImageReference1)),
        eq(false));
    mForwardingConsumer1.onNewResult(mIntermediateImageReference2, false);
    verify(mConsumer1).onNewResult(
        argThat(new CloseableReferenceMatcher(mIntermediateImageReference2)),
        eq(false));
    mForwardingConsumer1.onNewResult(mFinalImageReference1, true);
    verify(mConsumer1).onNewResult(
        argThat(new CloseableReferenceMatcher(mFinalImageReference1)),
        eq(true));
  ...
  public void testTwoIdenticalRequestAndOneDifferent() {
    ...
    verify(mConsumer3).onNewResult(
        argThat(new CloseableReferenceMatcher(mFinalImageReference2)),
        eq(true));
  ...
  private static class CloseableReferenceMatcher extends ArgumentMatcher<CloseableReference> {
    ...
    public boolean matches(Object argument) {
      if (argument == null) {
        ...
        return mCloseableImage.getUnderlyingReferenceTestOnly() ==
